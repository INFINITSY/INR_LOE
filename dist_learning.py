...
# inr model configs
parser.add_argument('--ckpt', type=str, default='save/20240427_235730_separate_std005/inr_loe_595.pt',
...
# latent configs
parser.add_argument('--latent_path', type=str, default='save/20240429_215023_compute_latents_30000/latents_train_e_596.pt')
parser.add_argument('--flat', type=bool, default=True, help='whether the latents are flat')
parser.add_argument('--subset', type=int, default=50, help='subset of the latents')

# training configs

parser.add_argument('--vae_batch_size', type=int, default=64, help='batch size')
parser.add_argument('--vae_epochs', type=int, default=5000, help='number of epochs')
parser.add_argument('--vae_lr', type=float, default=5e-3, help='learning rate')
parser.add_argument('--vae_hidden_dim', type=int, default=256, help='hidden layer dim')
parser.add_argument('--vae_latent_size', type=int, default=128, help='size of the latent space')
parser.add_argument('--epochs_log', type=int, default=10, help='log every n epochs')
parser.add_argument('--kld_weight', type=float, default=0.00025, help='weight of the kld loss')

args = parser.parse_args()

args.save = '/'.join(args.latent_path.split('/')[:-1])
# load latents 
dataset = LatentsDataset(args.latent_path, flat=args.flat, subset=args.subset)
...
input_size = dataset.latent_size[0]
# condition_size = dataset.latent_size[1]

# model = CVAE(input_size, condition_size, latent_size, hidden_size)
model = VAE(input_size, args.vae_latent_size, args.vae_hidden_dim).cuda()
...
for epoch in range(args.vae_epochs):
    ...
    for i, (data, _) in enumerate(dataloader):
        ...
        loss = loss_function(recon_batch, data, mu, logvar, args.kld_weight)
        loss.backward()
