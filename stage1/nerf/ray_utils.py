...
# https://github.com/bebeal/mipnerf-pytorch/blob/main/ray_utils.py
###################################################################
...
def sample_along_rays_mip(cam_rays, configs):
    """Stratified sampling along the rays.
    ...
    """
    # get configs
    num_samples = configs.num_samples_per_ray + 1
    near, far = configs.near, configs.far
    lindisp = configs.lindisp
    randomized = configs.randomized # noise
    ray_shape = configs.ray_shape

    # fixed direction
    rays_o, rays_d, radii = cam_rays[0], cam_rays[1], cam_rays[2, :, 0:1]

    return sample_along_rays(rays_o, rays_d, radii, num_samples, near, far, randomized, lindisp, ray_shape)
...
def volumetric_rendering(rgb, density, t_vals, dirs, white_bkgd):
    ...
    #delta = t_dists * torch.linalg.norm(dirs, dim=-1).unsqueeze(1)
