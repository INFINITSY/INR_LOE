...
class EncCombinerLinearCell(nn.Module):
    def __init__(self, Cin1, Cin2, Cout):
        super(EncCombinerLinearCell, self).__init__()
        self.fc = nn.Linear(Cin2, Cout, bias=True)
...
class DecCombinerLinearCell(nn.Module):
    def __init__(self, Cin1, Cin2, Cout, cell_type):
        super(DecCombinerLinearCell, self).__init__()
        self.cell_type = cell_type
        self.fc = nn.Linear(Cin1 + Cin2, Cout, bias=True)
        # self.fc = nn.Linear(Cin2, Cout, bias=True)
...
class ResMLPCell(nn.Module):
    def __init__(self, Cin, Cout, cell_type):
        super(ResMLPCell, self).__init__()
        self.cell_type = cell_type
        self.fc1 = nn.Linear(Cin, Cout, bias=True)
        self.fc2 = nn.Linear(Cout, Cout, bias=True)
        self.fc3 = nn.Linear(Cout, Cout, bias=True)
        self.fc4 = nn.Linear(Cout, Cout, bias=True)
        self.act = nn.ELU(inplace=True)

    def forward(self, x, res=0.1):
        skip = x
