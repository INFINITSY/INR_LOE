...
def kl_coeff_cycle_linear(step, total_step, min_kl_coeff=0.0001, max_kl_coeff=0.0005, 
                          n_cycle=4, ratio=0.5, constant_ratio=0.2):
    period = total_step / n_cycle
    # step = (max_kl_coeff - min_kl_coeff) / (period * ratio)  # linear increase per step in the increasing phase
    constant_step = constant_ratio * period
    step_increase = (max_kl_coeff - min_kl_coeff) / (period * ratio)
    
    # Calculate the position within the current cycle
    position_in_cycle = step % period

    if position_in_cycle <= constant_step:
        # Constant phase at the beginning of the cycle
        return min_kl_coeff
    elif position_in_cycle <= period * ratio + constant_step:
        # Increasing phase
        return min_kl_coeff + (position_in_cycle - constant_step) * step_increase
    else:
        # Constant phase at the max coefficient for the rest of the cycle
        return max_kl_coeff
...
def log_iw_mse(pred, target, log_q, log_p):
    recon = mse_loss(pred, target)
    return - recon - log_q + log_p
