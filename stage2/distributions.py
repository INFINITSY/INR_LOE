...
def soft_clamp5(x: torch.Tensor):
    return x.div(5.0).tanh_().mul(5.0)  # 5. * torch.tanh(x / 5.) <--> soft differentiable clamp between [-5, 5]
...
class DiscLogistic:
    ...
    def sample(self):
        u = torch.Tensor(self.means.size()).uniform_(1e-5, 1.0 - 1e-5).cuda()  # B, 3, H, W
        x = self.means + torch.exp(self.log_scales) * (torch.log(u) - torch.log(1.0 - u))  # B, 3, H, W
        x = torch.clamp(x, -1, 1.0)
        x = x / 2.0 + 0.5
        return x
...
class DiscMixLogistic:
    ...
    def log_prob(self, samples):
        ...
        mean2 = self.means[:, 1, :, :, :] + self.coeffs[:, 0, :, :, :] * samples[:, 0, :, :, :]  # B, M, H, W
        mean3 = (
            self.means[:, 2, :, :, :]
    ...
    def sample(self, t=1.0):
        gumbel = -torch.log(
            -torch.log(torch.Tensor(self.logit_probs.size()).uniform_(1e-5, 1.0 - 1e-5).cuda())
        )  # B, M, H, W
